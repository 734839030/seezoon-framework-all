		...
		if (!ShiroUtils.isSuperAdmin() && null != roles && !roles.isEmpty() && !mergeDataScope.contains(ALL)) {
			for (SysRole role : roles) {
				...
				if (CURRENT_DEPT_LOW_LEVEL.equals(dataScope)) {//本部门及以下
					...
					sb.append("create_by ").append("in (select sf_alias.id from (select id from sys_user where dept_id = '").append(deptId).append("') sf_alias) ");
				   ...
				   sb.append("create_by ").append("in (select sf_alias.id from (select su.id from sys_user su where su.dept_id in (select dept_id from sys_role_dept where role_id = '").append(role.getId()).append("')) sf_alias) ");
				}
			}
			//去掉最前面的or
